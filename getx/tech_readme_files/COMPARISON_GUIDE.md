...
class CounterController extends GetxController {
  final model = CounterModel().obs;
  
  void increment() => model.update((val) => val?.value++);
  void decrement() => model.update((val) => val?.value--);
  void reset() => model.update((val) => val?.value = 0);
}

// View
Obx(() => Text('${controller.model.value.value}'))
```
...
class CounterViewModel extends GetxController {
  ...
  void increment() => _counterValue.value++;
  void decrement() => _counterValue.value--;
  void reset() => _counterValue.value = 0;
}
...
```dart
class CounterController extends GetxController {
  final _counter = Rx<Counter?>(null);
...
// In View
Obx(() => Text('${controller.counter?.value ?? 0}'))
```

#### DDD
```dart
class CounterController extends GetxController {
  final _counter = Rx<CounterEntity?>(null);
...
// In View
Obx(() => Text('${controller.counter?.value.number ?? 0}'))
```
